...
Para que esse processor nao continue infinitamente caso o garoto nao morra, uma variabel denominada `contZeros` foi criada no arquivo `map.cpp`, essa variavel juntamente com a variavel `soma` no arquivo `ler.cpp` e as variaveis `fim_i` que recebe a coordenada i de inicio e `fim_j` que recebe a coordenada j de inicio e uma booleana `fin`, sao responsaveis por parar o garoto. A `soma` tem guardada a soma da quantidade de posicoes de todas as matrizes juntas. O `contZeros` recebe +1 se o valor da posica atual da matriz na funcao `movimento()` for igual a zero, e recebe zero toda vez que acha um valor maior que zero na posicao da matriz. Caso o mapa atual seja o primeiro, o contador de zeros for igual a soma e a posicao atual seja igual a posicao inicial, a booleana `fin` recebe true, e ao retornar a funcao `readOneMap()` encerra os movimentos do garoto significando que ele venceu o jogo.
